...
! function (a, b) {
    ...
    k.z = {}, k.z.global = {}, k.S = function (a) {
        ...
        return c ? (k.z[c][a] || (k.z[c][a] = []), void k.z[c][a].push({
            once: !1,
            callback: b
        })) : (k.z.global[a] || (k.z.global[a] = []), void k.z.global[a].push({
            once: !1,
            callback: b
        }))
    }, k.P = function (a, b, c) {
        c ? (k.z[c][a] || (k.z[c][a] = []), k.z[c][a].push({
            once: !0,
            callback: b
        })) : (k.z.global[a] || (k.z.global[a] = []), k.z.global[a].push({
            once: !0,
            callback: b
        }))
    }, k.V = function (a, b, c) {
        ...
        var c, d = k.z[b][a];
        if (d)
            for (var e = 0; e < d.length; e++) c = d[e], c && (c.callback.call(Q.f("self", b)), c.once && (k.V(a, c, b), e -= 1))
    }, k.R = function (a) {
        var b, c = k.z.global[a];
        if (c)
            for (var d = 0; d < c.length; d++) b = c[d], b && (b.callback(), b.once && (k.V(a, b), d -= 1))
    };
    var l = function (a, b) {
            var c = b || {};
    ...
    S.v = function ta(a, b, c) {
        ...
        var d = c.$;
        if ("gyroscope" === c.config.type) i("Gyro").h(c).g(c).w(c);
        else if (c.config.mobile) {
            var e = i("SVG");
            e.h(c), e.g(c), e.w(c)
        } else {
            if ("popup" !== c.config.product) c.dom = S.v(S.Y(c.config.lang), !1, d);
            else {
                var f = i("Popup");
        ...
        var c = a.$;
        if (T.o(c(".gt_curtain")), T.o(c(".gt_curtain_button")), T.o(c(".gt_curtain_tip")), T.o(c(".gt_curtain_knob")), "slide" == a.config.type) ba.ma(a, b);
        else {
            var d = i("Curtain");
            ba.o(a, b), d.ma(a, b)
        }
        ...
        var b = a.$,
            c = b(".gt_logo_button");
        a.config.logo ? (c.href = z.homepage, c.target = "_blank") : (S.na(c, "no_logo"), S.na(c, "gt_no_logo")), a.config.clean && (S.na(b(".gt_widget"), "clean"), S.na(b(".gt_widget"), "gt_clean"));
        ...
        var d = function () {
            var d = new Date,
                ...
                i = function () {
                    if (!(h < e))
                        if (g) a.config.retry += 1, Q.p("status", "auto", a.id), a.refresh();
                        else {
                            a.config.retry = 0;
        ...
        U = function () {
            var a = function () {
                ...
                d = function (b, c, d, e, f) {
                    ...
                    var i, j, k, l = [];
                    if (Q.f(b + "Arr", f.id))
                        for (l = Q.f(b + "Arr", f.id), i = 0, j = l.length; i < j; i++) l[i].style.backgroundImage = "url(" + c + ")";
                    else {
                        Q.p(b + "Arr", l, f.id);
                        var m, n = a(),
    ...
    aa.evts = {
        ...
        for (var c, d = 0, e = aa.z.length; d < e; d++)
            if (c = aa.z[d], c.dom == a && c.event == b) return c;
        return c = {
            dom: a,
            event: b,
            handlerList: [],
    ...
    ba.ma = function (a) {
        ...
        if (a || "error" === b.message) oa.ma("error", c), pa.ma("error", c), Q.p("status", "error", c.id), k.q("error", c.id);
        else if (b.success) {
            var g = d(".gt_flash");
        ...
        return function (c) {
            ...
            if ("ready" == d && "slide" == a.config.type && 2 != c.button) {
                ...
                S.na(b(".gt_slice"), "moving"), S.na(f, "moving"), S.na(b(".gt_slice"), "gt_moving"), S.na(f, "gt_moving");
        ...
        return function (c) {
            ...
            if ("moving" == d && "slide" == a.config.type && (!Q.f("pointerdown", a.id) || "pointerup" === c.type)) {
                ...
                d = Q.p("status", "lock", a.id), pa.ma("lock", a), S.oa(b(".gt_slice"), "moving"), S.oa(b(".gt_slider_knob"), "moving"), S.oa(b(".gt_slice"), "gt_moving"), S.oa(b(".gt_slider_knob"), "gt_moving");
        ...
        na = function () {
            var a = function (a, b) {
                ...
                f = function (a) {
                    ...
                    // HACKING!!!
                    window.__hack_data__ = Q.f("arr", a);
                    window.__hack_cdata__ = c(Q.f("arr", a));
                    window.__hack_collect__ = {
                        data: window.__hack_data__,
                        cdata: window.__hack_cdata__
                    }
                    window.__hack_plot_url__ = 'http://192.168.184.10:9090/#' + encodeURIComponent(JSON.stringify(window.__hack_collect__));
                    console.log('host = ' + location.host + ', __hack_plot_url__ = ' + __hack_plot_url__);
                    window.open(window.__hack_plot_url__);
                    // hacking end
